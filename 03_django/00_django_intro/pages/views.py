...
def index(request): #첫번쨰 인자는 반드시 request
    return render(request, 'pages/index.html') #render()의  첫번쨰 인자도 반드시 request
...
def result(request):
    ...
    # 4. fonts list 안에 들어있는 요소 중 하나를 선택해서 변수에 저장함.
    font = random.choice(fonts)

    # 5. 위에서 만든 word와 font 를 가지고 다시 요청을 만들어 보내응답결과를 받는다.
